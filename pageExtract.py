...
def get_info_by_page(url, proxy=None):
    ...
    try:
        if proxy:
            data = requests.get(url, headers=headers, proxies={"http": proxy},).content
        else:
    ...
    try:
        ...
        workName = {'title': soup.body.find('div',{'class':'titLeft'}).h1.text}
    except:
        return None
    ...
    data_line.update(workName)
    return data_line


def set_info(aic, a, proxy=None):
    if a['isGot']:
        return
    a_new = a
    url = a['url']
    info = get_info_by_page(url, proxy=proxy)
    if not info:
        return False
    a_new['isGot'] = True
    a_new.update(info)
    aic.replace_one({'_id': a['_id']}, a_new)
    #print info
    return True
